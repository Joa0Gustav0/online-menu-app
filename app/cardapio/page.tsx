...
function Page() {
  ...
  const productsCategories = [productsData.pizzas, productsData.burgers, productsData.acais]
  ...
  return (
    <main className="relative flex justify-center items-center w-full min-h-screen m-auto pt-[110px] pb-[36px] bg-[#f5f5f5] overflow-hidden">
      <div className="relative flex flex-col items-center gap-[26px] w-full max-w-[600px] z-[10]">
        ...
        <ul>
          {productsCategories.map((categorie) => categorie.map((product, i) => typeof product !== "string" ? (
            <MenuProduct key={"menu-product-" + i} filter={filter} param={categorie[0]} product={product} />
          ) : null ))}
        </ul>
      </div>
